...
const Tab = ({isActive, label, paragraph, onClick, progress, intervalDuration}: TabProps) => (
    <button
        ...
        className={`text-left text-lg flex flex-col group gap-1 transition-all ${
            isActive ? 'flex-[2] text-default' : 'flex-[1] text-secondary'
        }`}
        aria-selected={isActive}
        role="tab"
    >
        {/*TODO: add bg-{color}*/}
        <div className="relative w-full h-[2px] opacity-80 group-hover:opacity-100 rounded-full overflow-hidden">
            {isActive &&
                <motion.div
                    className={[
        ...
        <div className="md:h-20">
            ...
            {isActive && (
                <motion.p
                    ...
                    className="hidden md:block mt-1 text-secondary text-sm"
                >
                    {paragraph}
                </motion.p>
            )}
...
const TimedTabsSection = ({
    ...
    useEffect(() => {
        const progressIncrenment = (100 / intervalDuration) * (updateFrequency / 1000)

        controls.start(animation)

        const progressInterval = setInterval(() => {
            setProgress((prevProgres) => (prevProgres + progressIncrenment) % 101)
        }, updateFrequency)
    ...
    return (
        <SectionContainer className="flex flex-col gap-8 font-sans">
            <div className="flex flex-col lg:flex-row gap-8 xl:gap-10 justify-between">
                <div className="w-full lg:w-1/2 gap-2 flex flex-col items-start">
                    <h2 className="text-3xl xl:text-4xl max-w-[280px] md:max-w-xs xl:max-w-[360px] tracking-[-1px]">
                        {title}
                    </h2>
                    <p className="text-secondary mb-4 max-w-sm">{paragraph}</p>
                    {cta && (
                        <Button asChild type="button" size="sm" className="bg-[hsl(0,0%,93%)] text-card font-bold">
                            <Link href={cta.link}>
                                <ArrowUpRightIcon/>
                        ...
                        {tabs.map((tab, i) => (
                            <SwiperSlide key={i}>
                                <>
                                    ...
                                    />
                                    <CodeBlock hideCopy key={i} lang="py" size="large" background="#111">
                                        {tab.code}
                                    </CodeBlock>
                                </>
                            </SwiperSlide>
...
const OpenInColab = ({colabUrl, className}: { colabUrl: string; className?: string }) => (
    <Link
        ...
        target="_blank"
        className={[
            'flex items-center z-10 h-10 bg-[#111] hover:bg-[hsla(0,0%,100%,.02)] ease-in-out hover:text-secondary text-sm text-secondary shadow-lg hover:shadow-md rounded-full py-1 px-3 gap-2',
            className,
        ].join(' ')}
    >
        <Image
            className="opacity-100 hover:opacity-80 transition-opacity contrast-[.2] filter"
            src="/img/logos/google-colaboratory.svg"
